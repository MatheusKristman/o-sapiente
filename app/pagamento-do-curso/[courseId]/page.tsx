...
const formSchema = z
  .object({
    name: z.string().min(1, { message: "Nome obrigatório" }).min(4, { message: "Precisa ter no mínimo 4 caracteres" }),
    email: z.string().min(1, { message: "E-mail obrigatório" }).email({ message: "E-mail inválido" }),
    cpf: z.string().min(1, { message: "CPF obrigatório" }).min(14, { message: "CPF inválido" }),
    birthDate: z.date({
    ...
    addressNumber: z.string().min(1, { message: "Número do endereço obrigatório" }),
...
export default function CoursePaymentPage({ params }: { params: { courseId: string } }) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-full flex flex-col relative mt-24">
        <div className="w-full px-6 flex flex-col-reverse gap-12 sm:px-16 lg:container lg:mx-auto lg:grid lg:grid-cols-[1fr_500px]">
          <CoursePaymentForm form={form} isSubmitting={isSubmitting} />
          ...
          <div className="h-fit bg-[#E2EBEF] px-6 py-9 rounded-2xl flex flex-col gap-8 lg:sticky lg:top-10">
            ...
            <div className="w-full flex items-center justify-between">
              <span className="text-gray-primary text-base font-medium">{course.courseName}</span>

              <span className="text-gray-primary text-base font-medium">{formatPrice(course.price / 100)}</span>
            </div>
            ...
            <div className="w-full flex items-center justify-between">
              ...
              <span className="text-[#5A727D] text-base font-medium">{formatPrice(course.price / 100)}</span>
            </div>
            ...
            <div className="w-full flex items-center justify-between">
              ...
              <span className="text-[#37474F] text-lg font-semibold">{formatPrice(course.price / 100)}</span>
            </div>

            <CoursePaymentButtons isSubmitting={isSubmitting} className="hidden lg:flex" />
          </div>
        </div>
