...
const ProfilePhotoBox = () => {
  ...
  const { setProfilePhoto: setProfilePhotoUrl, profilePhoto: profilePhotoUrl, userId } = useUserStore();
  ...
  return (
    <div className="bg-white w-full p-9 rounded-2xl shadow-md shadow-[rgba(0,0,0,0.25)] flex flex-col gap-y-6 sm:max-w-[290px] lg:max-w-none">
      <div {...getRootProps()} className="relative aspect-square cursor-pointer rounded-2xl overflow-hidden group">
        <div
          className={cn(
        ...
        >
          <span className="text-lg text-white font-medium text-center">{MyAccountInfo.tipPhoto}</span>
        </div>
