...
const RequestDetailsModalResume = ({
  ...
  const { studentImage, studentName, subject, message, closeModal, reset } = useRequestDetailsModalStore();
  ...
  return (
    ...
    >
      <h3 className="text-3xl font-semibold text-gray-primary text-left mb-6">{requestDetailsInfo.title}</h3>
      ...
      <div className="w-full flex gap-x-2 mb-4">
        <span className="text-lg font-semibold text-green-primary">{requestDetailsInfo.subject}</span>
      ...
      {type === "Professor" && !plan && (
        <span className="mt-4 text-base text-gray-primary text-center max-w-md mx-auto">
          ...
          <Link onClick={handleCloseButton} href="/pagamento-do-plano" className="font-semibold text-green-primary">
